...
Accuracy$PARRF <- confusionmat.PARRF['Accuracy']  
...
Accuracy$glmboost <- confusionmat.glmboost['Accuracy']  
...
Accuracy$glmnet <- confusionmat.glmnet['Accuracy']  
...
ExperimentResults <- cbind(as.data.frame(matrix(c(AUC,Accuracy,LogLoss),nrow = 14, ncol = 3, 
                           dimnames = list(row.names, col.names)))) ## make nrows and ncols bigger when more models created

ExperimentResults

# TODO unpacking variables of importance from best performing model - SVM
```
