...
skimr::skim(wrk.combined_datatypecleaned)
...
tsne <- Rtsne::Rtsne(x, dims = 2, perplexity=9, verbose=TRUE, max_iter = 500)
...
SpecialControl <- trainControl(method = "repeatedcv",
                           number = 10,
                           repeats = 10,
...
- 2. Random Forest, using default tuning
- 3. Boosted tree model with tuning (grid search using learning rate and number of trees)
- 4. Stochastic gradient boosting (complex)
...
```{r eval=, echo=TRUE, message=FALSE, warning=FALSE}
set.seed(1111)
# Let’s build our five basic models:
# 1) linear algorithms
# LDA
#set.seed(7)
fit.lda <- caret::train(TARGET_heart_disease_present~., data=trainData2, method="lda", metric=metric, trControl=control)

# 2) nonlinear algorithms
# CART
#set.seed(7)
fit.cart <- caret::train(TARGET_heart_disease_present~., data=trainData2, method="rpart", metric=metric, trControl=control)

# 3) kNN
#set.seed(7)
fit.knn <- caret::train(TARGET_heart_disease_present~., data=trainData2, method="knn", metric=metric, trControl=control)

# 4) advanced algorithms
# SVM 
#set.seed(7)
# NOTE: Using "trainData2" with factors configured with names, and train control is using SpecialControl object
fit.svm <- caret::train(make.names(TARGET_heart_disease_present)~., data=trainData2, method="svmRadial", metric=metric, trControl = SpecialControl)
...
#Random Forest
#set.seed(7)
fit.rf <- caret::train(TARGET_heart_disease_present~., data=trainData2, method="rf",preProcess = c("scale", "center"), metric=metric, trControl=control)

#==================================== General Performing Algorithms =================================================
#GLM
fit.glm <- caret::train(TARGET_heart_disease_present~., data=trainData2, method = "glm", metric=metric, trControl=control)

# Random forest with default tuning
fit.rf_default <- randomForest(TARGET_heart_disease_present ~ .,
                        data = trainData2,
                        importance = TRUE,
                        ntree = 2000)
...
fit.boostedtree <- caret::train(TARGET_heart_disease_present ~ .,data=trainData2, method = 'gbm',
...
# Neural network
fit.nnet <- caret::train(TARGET_heart_disease_present~., data=trainData2, method = "nnet", metric=metric, trControl=control)
...
fit.avnnet <- caret::train(TARGET_heart_disease_present~., data=trainData2, method = "avNNet", metric=metric, trControl=control)

#Tree-Based ensembles
fit.nodeHarvest <- caret::train(TARGET_heart_disease_present~., data=trainData2, method = "nodeHarvest", metric=metric, trControl=control)

# Parallel random forest
fit.parRF <- caret::train(TARGET_heart_disease_present~., data=trainData2, method = "parRF", metric=metric, trControl=control)

# mboost
fit.glmboost <- caret::train(TARGET_heart_disease_present~., data=trainData2, method = "glmboost", metric=metric, trControl=control)
fit.glmnet   <- caret::train(TARGET_heart_disease_present~., data=trainData2, method = "glmnet", metric=metric, trControl=control)
...
# svm=fit.svm not included here, "there are different numbers of resamples" in this model versus the rest

results <- caret::resamples(list(lda=fit.lda, cart=fit.cart, knn=fit.knn, fit.svm, rf=fit.rf, 
                                 glm=fit.glm, rfdef=fit.rf_default, boostedtree=fit.boostedtree, gbm=fit.gbm,
                                 nnet=fit.nnet, avnnet=fit.avnnet, nodeharvest=fit.nodeHarvest, parRF=fit.parRF,
                                 glmboost=fit.glmboost, glmnet=fit.glmnet ))
                      #xgboost=fit.caret.xgboost, gbmh2o=fit.gbm.h2O , glmnet_h2o=fit.glmnet_h2o
summary(results)
...
# LOGLOSS:
#$`logReg`
#[1] 0.4693868  // 0.36453 comp result
#$RF
#[1] 0.7053043  // 0.4028 comp result
#$CaretRF
#[1] 0.7366456  // 0.40 comp result
#$boost
#[1] 1.156777
#$gbm
#[1] 0.6511815
#$svm
#[1] 0.4587091 << best model! // 0.39 comp result


# ===============================================================================================================#
# SELECTED BEST PERFORMING MODEL: VARIABLES OF IMPORTANCE =======================================================#
# ===============================================================================================================#
summary(logRegModel)$coeff # Second best performing mode, GLM, variables of importance. TODO: recode sex as factor.

RFModel$confusion
imp <- RFModel$importance
imp[order(imp, decreasing = TRUE), ]

# TODO: plot variables of importance --- try lime, boruta (plot is currently showing Error vs Trees)
RFImp = varImp(RFModel, scale = FALSE)
row = rownames(varImp(RFModel, scale = FALSE)$importance)
row = convert.names(row)
rownames(RFModel$importance)=row
plot(RFModel,main = 'Variable importance for heart failure prediction with boosted tree') 

# Ideas REF: https://rpubs.com/cgalea/396365
library(spFSR)
spFSR::plotImportance(RFModel)

# Ideas REF: https://shiring.github.io/machine_learning/2017/03/31/webinar_code
library(rpart)
library(rpart.plot)
rpart.plot(RFModel, extra = 100)
...
library(pROC)
AUC$NNET <- roc(as.numeric(validateData$TARGET_heart_disease_present),as.numeric(as.matrix((PredictionProb.nnet))))$auc
Accuracy$NNET <- confusionmat.nnet$overall['Accuracy']  
library(Metrics)
LogLoss$NNET <- ModelMetrics::logLoss(as.numeric(validateData$TARGET_heart_disease_present), as.numeric(as.matrix(PredictionProb.nnet)))

AUC$NNET
Accuracy$NNET
LogLoss$NNET
# AUC 0.8472, ACC 0.722, LogLoss 1.2


#========== GBM ===================================== #
# estimate skill of GBM on the validation dataset
Prediction_GBM <- predict(fit.gbm, validateData) # For confusion matrix
PredictionProb.gbm <- predict(fit.gbm, validateData, type = "prob")[,2] # FOR probability outputs, [] for binary predcition, we will have 2 pred columns
confusionmat.gbm <- confusionMatrix(Prediction_GBM, validateData[,"TARGET_heart_disease_present"])

library(pROC)
AUC$GBM <- roc(as.numeric(validateData$TARGET_heart_disease_present),as.numeric(as.matrix((PredictionProb.gbm))))$auc
Accuracy$GBM <- confusionmat.gbm$overall['Accuracy']  
library(Metrics)
LogLoss$GBM <- logLoss(as.numeric(validateData$TARGET_heart_disease_present), as.numeric(as.matrix(PredictionProb.gbm)))
# AUC 0.8424, ACC 0.7777778 , LogLoss 1.175429
...
SOLUTION_GBM <- data.frame(patient_id = mod.test_ID, TARGET_heart_disease_present = SUBMISSION_Prediction_GBM$`1`)
...
library(pROC)
AUC$LDA <- roc(as.numeric(validateData$TARGET_heart_disease_present),as.numeric(as.matrix((PredictionProb.lda))))$auc
Accuracy$LDA <- confusionmat.lda$overall['Accuracy']  
library(Metrics)
LogLoss$LDA <- logLoss(as.numeric(validateData$TARGET_heart_disease_present), as.numeric(as.matrix(PredictionProb.lda)))
# AUC 0.8542, ACC 0.7592593 , LogLoss 1.928269
...
library(pROC)
AUC$NodeHarvest <- roc(as.numeric(validateData$TARGET_heart_disease_present),as.numeric(as.matrix((PredictionProb.nodeharvest))))$auc
Accuracy$NodeHarvest <- confusionmat.nodeharvest$overall['Accuracy']  
library(Metrics)
LogLoss$NodeHarvest <- logLoss(as.numeric(validateData$TARGET_heart_disease_present), as.numeric(as.matrix(PredictionProb.nodeharvest)))
# AUC 0.7896, ACC 0.6666667 , LogLoss 1.313229
...
Prediction_SVM <- predict(fit.svm, validateData) # For confusion matrix
PredictionProb.SVM <- predict(fit.svm, validateData, type = "prob")[,2] # FOR probability outputs, [] for binary predcition, we will have 2 pred columns
recode_factor(Prediction_SVM, "X0" == "0", "X1" == "1", .default = "D", .missing = "M")
confusionmat.SVM <- confusionMatrix(Prediction_SVM, validateData[,"TARGET_heart_disease_present"])

library(pROC)
AUC$SVM <- roc(as.numeric(validateData$TARGET_heart_disease_present),as.numeric(as.matrix((PredictionProb.SVM))))$auc
Accuracy$SVM <- confusionmat.SVM$overall['Accuracy']  
library(Metrics)
LogLoss$SVM <- logLoss(as.numeric(validateData$TARGET_heart_disease_present), as.numeric(as.matrix(PredictionProb.SVM)))
...
ExperimentResults <- cbind(as.data.frame(matrix(c(AUC,Accuracy,LogLoss),nrow = 4, ncol = 3, 
